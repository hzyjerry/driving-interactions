...
def grad(f, x, constants=[]):
    ret = th.gradient.grad(
        f, x, consider_constant=constants, disconnected_inputs="warn"
    )
...
class NestedMaximizer(object):
    def __init__(self, f1, vs1, f2, vs2):
        ...
        self.sz2 = [
            (0 if i == 0 else self.sz2[i - 1], self.sz2[i])
            for i in range(len(self.sz2))
        ]
        ...
        self.func1 = th.function(
            self.new_vs1,
            [-self.f1, -self.df1],
            givens=zip(self.vs1, self.new_vs1),
        ...
        self.func2 = th.function([], [-self.f2, -self.df2], on_unused_input="ignore")
...
class Maximizer(object):
    def __init__(
        ...
        self.sz = [
            (0 if i == 0 else self.sz[i - 1], self.sz[i]) for i in range(len(self.sz))
        ]
        ...
        self.func = th.function(
            self.new_vs,
            [-self.f, -self.df],
            givens=zip(self.vs, self.new_vs),
            on_unused_input="ignore",
        )
        ...
        def f_and_df(x0):
            ...
            for _ in self.gen():
                ...
                if (
                    np.isnan(res[0]).any()
                    or np.isnan(res[1]).any()
                    or (np.abs(res[0]) > self.inf_ignore).any()
                    or (np.abs(res[1]) > self.inf_ignore).any()
                ):
                    continue
                if s is None:
                    s = res
    ...
    def argmax(self, vals={}, bounds={}):
        ...
        for v, (a, b) in zip(self.vs, self.sz):
            if v in bounds:
                ...
                B += [(None, None)] * (b - a)
        x0 = np.hstack(
            [np.asarray(vals[v]) if v in vals else v.get_value() for v in self.vs]
        )
        if self.method == "bfgs":
            ...
            opt = scipy.optimize.minimize(
                self.f_and_df, x0=x0, method=self.method, jac=True
            ).x
...
if __name__ == "__main__":
    ...
    x1 = vector(2)
    x2 = vector(1)
    f1 = -((x1[0] - x2[0] - 1) ** 2 + (x1[1] - x2[0]) ** 2) - 100.0 * tt.exp(
        40.0 * (x1[0] - 4)
    )
    f2 = -((x1[0] - 2.0) ** 2 + (x1[1] - 4.0) ** 2) - (x2[0] - 6.0) ** 2
    optimizer = NestedMaximizer(f1, [x1], f2, [x2])
    optimizer.maximize(bounds=[(0.0, 10.0)])
    print(x2.get_value())
    print(x1.get_value())
